...
def fetch_articles_for_date(db: Session, target_date: datetime, category: str = None):
    """
    ...
    Args:
        ...
        category: Optional category filter (e.g., 'business', 'technology', 'news')
    """
    # Use the DB's date extraction to match all rows whose parsed_at date equals target_date.date()
    query = db.query(Article).filter(func.date(Article.parsed_at) == target_date.date())
    ...
    query = db.query(Article).filter(func.date(Article.published_at) == target_date.date())
...
def process_daily_summary(db: Session, date: datetime = None, category: str = None) -> DailySummary:
    """
    Create a summary for articles from a specific date and optionally filtered by category.
    If date is not provided, process for today.
    
    Args:
        db: Database session
        date: Target date for summarization (defaults to yesterday)
    ...
    # Fetch articles for the calendar date (with optional category filter)
    articles = fetch_articles_for_date(db, target_date, category=category)

    if not articles:
        category_str = f" in category '{category}'" if category else ""
        logger.warning(f"No articles found for {target_date.date()}{category_str}")
        return None

    category_str = f" in category '{category}'" if category else ""
    logger.info(f"Processing {len(articles)} articles{category_str} for {target_date.date()}")
    ...
    logger.info(f"Generated summary for {target_date.date()}{category_str} with {len(articles)} articles")

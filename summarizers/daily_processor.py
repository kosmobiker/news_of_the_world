...
def fetch_articles_for_date(db: Session, target_date: datetime, category: str = None, days: int = 1):
    """
...
def process_daily_summary(db: Session, date: datetime = None, category: str = None, days: int = 1) -> DailySummary:
    """
    ...
    if not articles:
        ...
        start_date = target_date - timedelta(days=days)
        logger.warning(f"No articles found from {start_date.date()} to {target_date.date()}{category_str}")
        return None

    category_str = f" in category '{category}'" if category else ""
    start_date = target_date - timedelta(days=days)
    logger.info(f"Processing {len(articles)} articles{category_str} from {start_date.date()} to {target_date.date()}")
    ...
    logger.info(f"Generated summary for {start_date.date()} to {target_date.date()}{category_str} with {len(articles)} articles")
